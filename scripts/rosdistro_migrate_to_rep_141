...
def generate_repos_url(repos_url, doc_url, release_url, source_url):
        ...
        if 'packages' in release_repo_data:
            ...
            for pkg_name in pkgs:
                if pkgs[pkg_name] is not None:
                    assert not 'status' in pkgs[pkg_name]
                    assert not 'status_description' in pkgs[pkg_name]
    ...
    for repo_name in doc_data['repositories']:
        ...
        if not repo_name in data['repositories']:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Migrate the distros to the new spec.')
